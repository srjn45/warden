...
func TestDetectRateLimit_ClaudeCodeSessionLimit_FutureTime(t *testing.T) {
	// Generate a time 2 hours in the future
	loc, _ := time.LoadLocation("Europe/Madrid")
	now := time.Now().In(loc)
	futureTime := now.Add(2 * time.Hour)
	futureTimeStr := futureTime.Format("3:04pm")

	pane := fmt.Sprintf(`You've hit your session limit · resets %s (Europe/Madrid)`, futureTimeStr)
	...
	require.True(t, restoreTime.After(now), "should be in the future")
	require.Equal(t, loc, restoreTime.Location())
}

func TestDetectRateLimit_ClaudeCodeSessionLimit_PastTime(t *testing.T) {
	// Use a time that's definitely in the past (1:30am when it's likely afternoon/evening)
	pane := `You've hit your session limit · resets 1:30am (Europe/Madrid)`
	...
	// When time is in the past, should return current time for immediate retry
	now := time.Now()
	timeDiff := restoreTime.Sub(now).Abs()
	require.Less(t, timeDiff, 2*time.Second, "should return current time when reset time is in the past")
}

func TestDetectRateLimit_24HourFormat(t *testing.T) {
	// Generate a time 3 hours in the future in 24-hour format
	loc, _ := time.LoadLocation("Europe/Madrid")
	now := time.Now().In(loc)
...
func TestDetectRateLimit_AMTime(t *testing.T) {
	// Generate a time 1 hour in the future
	loc, _ := time.LoadLocation("America/New_York")
	now := time.Now().In(loc)
	futureTime := now.Add(1 * time.Hour)
	futureTimeStr := futureTime.Format("3:04pm")
	if futureTime.Hour() < 12 {
		futureTimeStr = futureTime.Format("3:04am")
	}

	pane := fmt.Sprintf(`You've hit your session limit · resets %s (America/New_York)`, futureTimeStr)
...
func TestDetectRateLimit_UsageLimitKeyword(t *testing.T) {
	...
	pane := fmt.Sprintf(`Usage limit exceeded · resets %s (Europe/London)`, futureTimeStr)
...
func TestDetectRateLimit_QuotaExceeded(t *testing.T) {
	// Generate a time 3 hours in the future
	...
	pane := fmt.Sprintf(`Quota exceeded · resets %s (UTC)`, futureTimeStr)
