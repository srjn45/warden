...
func TestNewRouterFromConfig_ExplicitTier(t *testing.T) {
	// An explicit local_llm.tier overrides the model-name heuristic. A T2 model
	// plans even a multi-clause ("pipeline") request locally.
	r := NewRouterFromConfig(config.Config{LocalLLM: config.LocalLLMConfig{Tier: "T2", Model: "tiny"}}, nil)
	require.NotNil(t, r)
...
func TestNewRouterFromConfig_DegradesWhenUnderTierAndEscalateOff(t *testing.T) {
	...
	r := NewRouterFromConfig(config.Config{LocalLLM: config.LocalLLMConfig{Tier: "T0", Escalate: false}}, nil)
...
func TestNewRouterFromConfig_ModelClassifierWired(t *testing.T) {
	// local_llm.classifier: model routes on the model's verdict. A trivial-looking
	...
	cfg := config.Config{LocalLLM: config.LocalLLMConfig{Tier: "T0", Escalate: false, Classifier: "model"}}
...
func TestNewRouterFromConfig_DefaultsToHeuristicIgnoringComp(t *testing.T) {
	// With local_llm.classifier unset, a present Completer is NOT consulted: the
	// heuristic scores "fix the bug" T0, so a T0 model plans locally even though
	// the (unused) model would have said T2.
	cfg := config.Config{LocalLLM: config.LocalLLMConfig{Tier: "T0", Escalate: false}}
	r := NewRouterFromConfig(cfg, fakeCompleter{out: "T2"})
...
func TestNewRouterFromConfig_FallsBackToModelTier(t *testing.T) {
	...
	r := NewRouterFromConfig(config.Config{LocalLLM: config.LocalLLMConfig{Model: "llama3"}}, nil)
	require.NotNil(t, r)
