...
func TestClientListModels(t *testing.T) {
	var c capture
	ts := jsonServer(t, &c, 0, `[{"backend_id":"claude","model_id":"claude-3-7-sonnet","tier":"tier-2","display_name":"Claude 3.7 Sonnet","enabled":true}]`)
	cl := New(ts.URL)
	...
	require.Equal(t, "claude-3-7-sonnet", models[0].ModelID)
...
func TestClientSetModelTier(t *testing.T) {
	var c capture
	ts := jsonServer(t, &c, 0, `{"backend_id":"claude","model_id":"claude-3-7-sonnet","tier":"tier-1","display_name":"Claude 3.7 Sonnet","enabled":true}`)
	cl := New(ts.URL)

	m, err := cl.SetModelTier(context.Background(), "claude", "claude-3-7-sonnet", "tier-1")
	require.NoError(t, err)
	require.Equal(t, http.MethodPut, c.method)
	require.Equal(t, "/api/v1/models/claude/claude-3-7-sonnet/tier", c.path)
