...
func NewServer(daemonBase string) *Server {
	...
	mcpsdk.AddTool(s.mcp, &mcpsdk.Tool{
		...
		Description: "Create a DAG pipeline of agent jobs from a YAML spec (the daemon parses, validates, and stores it). Use this to drive a multi-stage / dependent agent workflow (e.g. analyze→implement→review) instead of spawning and wiring agents by hand. When you call this as an orchestrator, the pipeline records you as its owner (owner_id, the pipeline-mode analogue of a spawned agent's parent_id) so a delegated pipeline knows who to report back to. Delegated monitoring: set `notify_owner: true` in the spec to subscribe as owner, and mark any job `callback: true` to make it a decision point — warden then push-wakes you (a directed message you read via wait_for_message) at each callback job and once on completion, so you block instead of polling and spend tokens only at real decisions. A pure DAG (no callbacks) wakes you just once, at the end. The pipeline starts in `pending` — call start_pipeline to spawn its entry jobs. Returns the created pipeline {id, status, owner_id, jobs}.",
