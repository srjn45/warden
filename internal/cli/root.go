...
func newRootCmd() *cobra.Command {
	...
	root.AddCommand(newProjectCmd(), newProjectsCmd(), newProjectGroupsCmd(), newWorkspaceCmd())
	...
	SetCommandHelpMetadata(factoryResetCmd, "operate", 50, "warden factory-reset", "", NodeLeaf)
	root.AddCommand(factoryResetCmd)
	setupCmd := newSetupCmd()
	markEntryPoint(setupCmd, 10)
	loginCmd := newLoginCmd()
	markEntryPoint(loginCmd, 5)
