...
// (`n`) and the default terminal launch in that dir. When native is set, the
// cockpit is laid out as a window in the user's *current* tmux session (for when
// warden is launched from inside tmux, where the classic cockpit's `tmux attach`
// can't nest) instead of building its own session to attach to.
func runCockpit(a *client.Client, native bool) error {
...
// It matches the daemon's build parameters (home cwd) so the rebuilt session is
// byte-for-byte what an attach would have produced.
func rebuildWebCockpitSession() error {
	...
	if _, err := tui.EnsureWebCockpit(context.Background(), run, self, home, true); err != nil {
