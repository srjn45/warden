...
func TestModelsListByTier(t *testing.T) {
	...
	require.Contains(t, out, "claude-opus")
	require.Contains(t, out, "claude-3-7-sonnet")
}
...
func TestModelsListFilterTier(t *testing.T) {
	...
	require.NoError(t, err)
	require.Contains(t, out, "claude-opus")
	require.NotContains(t, out, "claude-3-7-sonnet")
...
func TestModelsTierSet(t *testing.T) {
	...
	out, err := runGit(t, "127.0.0.1:0", "models", "tier", "claude", "claude-3-7-sonnet", "tier-1")
	require.NoError(t, err)
	require.Contains(t, out, "model claude/claude-3-7-sonnet tiered as tier-1")

	m, err := st.GetModel("claude", "claude-3-7-sonnet")
	require.NoError(t, err)
...
func TestModelsTierInvalid(t *testing.T) {
	...
	_, err := runGit(t, "127.0.0.1:0", "models", "tier", "claude", "claude-3-7-sonnet", "tier-invalid")
	require.Error(t, err)
	require.Contains(t, err.Error(), "invalid tier")
}
