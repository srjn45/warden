...
// runCLI drives a wd subcommand against a stub daemon at addr with the given
// stdin, returning stdout+stderr.
func runCLI(t *testing.T, addr, stdin string, args ...string) (string, error) {
	t.Helper()
	root := newRootCmd()
...
func TestExportEnvelopeShape(t *testing.T) {
	...
	out, err = runCLI(t, addr, "", "export", "--all")
	require.NoError(t, err)
	require.NoError(t, json.Unmarshal([]byte(out), &env))
...
func TestExportImportRoundTrip(t *testing.T) {
	...
	dump, err := runCLI(t, addr, "", "export")
	require.NoError(t, err)

	out, err := runCLI(t, addr, dump, "import")
	require.NoError(t, err)
...
func TestImportRejectsMalformedJSON(t *testing.T) {
	...
	_, err := runCLI(t, addr, "{not json", "import")
	require.Error(t, err)
