// Package projectkey is the single canonical normalizer that maps a repository
// to a stable identity key (design §4.1 / Track B's B2). It lives in its own
// leaf package — depending on nothing inside warden — so every layer can key on
// the same project identity: the daemon (group membership, summaries), and the
// TUI (grouping agents by project) both import it. Keeping it a leaf is what
// lets the TUI reuse it: the daemon package imports the TUI (attach.go), so the
// normalizer could not live in package daemon without an import cycle.
package projectkey
...
// Key returns a stable identity key for a repository. When remoteURL normalizes
// to a canonical remote key that key is returned (so two worktrees of the same
// repo collapse to one key); otherwise a `local:` fallback derived from repoRoot
// is returned — never an empty/rejected result. This is the single project-key
// normalizer referenced by the design (§4.1) and the group store (Track B).
func Key(remoteURL, repoRoot string) string {
...
// ForDir resolves the project key for a working directory by reading its
// canonical git remote (origin), falling back to a `local:` key rooted at the
// repo's top level (or dir itself when that cannot be determined). It is the
// git-plumbing entry point that pairs with the pure normalizer above.
func ForDir(ctx context.Context, dir string) string {
