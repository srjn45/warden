...
func TestGitCommitPinsToSessionWorkdir(t *testing.T) {
	...
	resp, err := http.Post(ts.URL+"/git/commit", "application/json", bytes.NewReader(body))
...
func TestGitCommitNoDirNoSessionRejected(t *testing.T) {
	...
	defer ts.Close()
	resp, err := http.Post(ts.URL+"/git/commit", "application/json", strings.NewReader(`{"message":"m"}`))
	require.NoError(t, err)
	defer resp.Body.Close()
...
func TestGitPush(t *testing.T) {
	...
	resp, err := http.Post(ts.URL+"/git/push", "application/json", bytes.NewReader(body))
	require.NoError(t, err)
	defer resp.Body.Close()
	require.Equal(t, http.StatusOK, resp.StatusCode)
...
func TestGitSyncReturnsConflicts(t *testing.T) {
	...
	resp, err := http.Post(ts.URL+"/git/sync", "application/json", bytes.NewReader(body))
	require.NoError(t, err)
	defer resp.Body.Close()
	require.Equal(t, http.StatusOK, resp.StatusCode)
