...
// attachFreshMbox wires a brand-new temp-dir mailbox to srv and returns it.
// Use this AFTER seating agents when the test only cares about messages produced
// by the operation under test — B4's brokerIntroductions fires during JoinGroup,
// so seating before attaching the mbox keeps the inbox clean for the assert.
func attachFreshMbox(t *testing.T, srv *Server) *mailbox.Store {
	t.Helper()
	mb, err := mailbox.New(t.TempDir())
	if err != nil {
		t.Fatalf("attachFreshMbox: %v", err)
	}
	t.Cleanup(func() { mb.Close() })
	srv.mbox = mb
	return mb
...
func TestTerminateGroupedWithoutConfirmIsGated(t *testing.T) {
	...
	seatTwo(t, srv, fs)
	mb := attachFreshMbox(t, srv) // isolate from B4 intro messages during seatTwo
...
func TestAbandonmentNoticeText(t *testing.T) {
	...
	require.NoError(t, err)
	mb := attachFreshMbox(t, srv) // isolate from B4 intro messages during joins
