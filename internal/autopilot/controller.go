...
func (c *Controller) Enable(ctx context.Context) (Status, error) {
	...
	for _, file := range c.plans {
		...
		if r.skipComplete {
			...
			// is neither a preflight failure nor a fresh run. Surface the skip so the
			// owner sees the run completed rather than silently vanishing, then leave
			// it out of the active run set (it never claims its repo, so a new plan may
			// take over the same repo).
