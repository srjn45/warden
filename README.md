...
- **Collaboration groups, pipeline-as-substrate & a projects-first cockpit** — three related shifts. **Collaboration groups** make one developer's per-project **orchestrator** agents mutually discoverable so they message and delegate across projects: `warden collaborate group <name> join|leave` (MCP `collaborate_group`) seats **one orchestrator per project** (project = the canonical git remote, with a `local:` fallback; a duplicate join returns the seated agent), resolves a one-line project summary, and has **warden broker introductions** both directions at zero agent tokens — leaving is soft (in-flight replies still deliver), only *terminate* orphans work. **Delegated monitoring** repositions the pipeline as warden's shared lifecycle substrate: an orchestrator that calls `create_pipeline` is recorded as its `owner_id`, and with spec `notify_owner: true` + per-job `callback: true` warden **push-wakes** it (a directed message read via `wait_for_message`) only at real decision points — it **blocks instead of polling**, spending tokens on nothing mechanical. Workers declare completion with `wd job done --summary` (or a `<<WARDEN_DONE>>` sentinel), and a pane-state classifier + wall-clock watchdog catch stuck/idle workers out-of-band. The **TUI cockpit goes projects-first**: a **Projects** frame groups agents (and their nested pipelines) by git project, a flat **Terminals** frame stays as-is, the top-level Approvals/Pipelines sections are gone, and **`o`** opens a full-pane **Open Project** panel (recent list · open-local · git-clone into `~/.warden/workspace/<project>`) that auto-spawns the project's single orchestrator. The **web** app gains a flat **Agents** tab (`/agents`, master-detail like Terminals). See [Collaboration groups](https://srjn45.github.io/warden/multi-agent/collaboration-groups/) and [Pipelines](https://srjn45.github.io/warden/multi-agent/pipelines/).
...
`warden tui` (or bare `warden`) opens a **tmux-composited cockpit** — a dedicated tmux session with three panes: the **control** pane (top-left), a **terminal** pane (bottom-left), and a full-height **agent** pane (right). The control pane is **projects-first**: two bordered frames — **Projects** (agents grouped under their git **Project** node, with each agent's subagents and pipelines nesting beneath it) and **Terminals** (a flat, non-project-scoped list) — that you browse freely with `↑`/`↓` without disturbing the viewports; press `Enter` to open the selected entity. The old top-level **Approvals and Pipelines sections are gone**: a blocked agent surfaces via its **needs-input** status (`p` answers it), and every pipeline renders inside Projects (a delegated pipeline under its owning orchestrator, a human one under its project node). Opening an **agent** attaches its interactive `claude` session in the agent pane (right); opening a **terminal** attaches it in the terminal pane (bottom-left). The cockpit always keeps at least one terminal — a default terminal in the launch directory opens on startup. Press **`o`** for the full-pane **Open Project** panel (recent list · `l` open local · `g` git-clone into `~/.warden/workspace/<project>`), which auto-spawns the project's single orchestrator.
...
| `o` | **Open Project** — full-pane panel: recent list · `l` open local · `g` open via git (clone into `~/.warden/workspace/<project>`); opening auto-spawns/focuses the project's single orchestrator; `Esc` back |
...
| `?` | Toggle help overlay |
| `Alt+t` / `Alt+a` | Rotate a viewport — `Alt+t` cycles the terminal pane over terminals, `Alt+a` cycles the agent pane over the Projects frame's agents. Each grabs focus on the target pane. Global (works from any pane, even while typing). `Alt+p` is retired — pipelines live inside Projects |
| `Alt+Shift+t` / `Alt+Shift+a` | Same rotations in **reverse** |
| `Ctrl-b` then `t`/`a` | Config-free rotation fallback (add Shift for reverse) — identical to `Alt+t/a` but via the tmux prefix, for terminals that don't send Alt/Option as Meta (**macOS Terminal.app / iTerm2** default) |
| `◆` | Marks the agent/terminal currently shown in a pane (not a key) — its name carries a bold magenta badge in the control tree, tracking both `Enter`-open and the `Alt+t`/`Alt+a` rotation |
| `q` | Quit and tear down the cockpit |

Move focus between panes with **Alt+←/→/↑/↓** (no tmux prefix); rotate the terminal/agent viewports with **Alt+t / Alt+a** (add **Shift** to rotate in reverse; `Alt+p` is retired — pipelines live inside the Projects frame). Rotation grabs focus on the pane it drives, so cycling agents drops you straight into the session. On **macOS**, Terminal.app and iTerm2 don't send Option as Meta by default, so `Option+t/a` won't fire — use the config-free `Ctrl-b` prefix fallback (`Ctrl-b` then `t`/`a`, Shift for reverse), or enable "Use Option as Meta key". The row currently docked in a pane is marked with a **◆** and a bold **name badge** in the control tree, following both `Enter`-open and the Alt rotation. See [docs/USAGE.md §7](docs/USAGE.md) for the full cockpit guide and caveats around nested tmux.
...
Jobs can be made conditional with `run_if: success` (default) `| failure | always` — e.g. a `run_if: failure` rollback/notify step that only runs when an upstream job fails. Pipelines render inside the TUI **Projects** frame (under their owning orchestrator, or under their project node) and in the web **Pipelines** tab. See [docs/USAGE.md](docs/USAGE.md) for the full authoring guide.

**Delegated monitoring (push, not poll).** A pipeline is warden's shared lifecycle substrate at three levels — direct (human), **delegated** (an orchestrator), and autopilot. When an orchestrator calls `create_pipeline` it's recorded as the pipeline's `owner_id`; add spec `notify_owner: true` and mark decision-point jobs `callback: true`, and warden **push-wakes** the owner (a directed message read via `wait_for_message`) at each callback and once on completion — so the orchestrator **blocks instead of polling** and spends tokens only at real decisions (a pure DAG wakes it just once, at the end). A worker declares completion in one shot with **`wd job done --summary "…"`** (`--status success|failure|blocked`; outside a pipeline it prints a `<<WARDEN_DONE>>{json}` sentinel), so warden closes the job without an interrogation turn.

### Collaboration groups — `warden collaborate group`

Make one developer's per-project **orchestrator** agents mutually discoverable so they message and delegate across projects — a roster + introduction broker, not a new channel (peers talk over the existing `warden msg` bus).

```sh
warden collaborate group my-team join    # seat this agent as its project's orchestrator
warden collaborate group my-team leave   # remove the seat (soft — in-flight replies still deliver)
```

`join` creates the group if absent, enforces **one orchestrator per project** (project = canonical git remote, `local:` fallback for a remoteless repo; a duplicate join returns the seated agent), flips the caller to the **orchestrator** role, resolves a one-line project summary, and has **warden broker introductions** both directions at zero agent tokens. `leave` is soft; only **terminate** orphans in-flight work (and is gated by a confirmation naming the group). MCP mirror: `collaborate_group {group, action}`. See [Collaboration groups](https://srjn45.github.io/warden/multi-agent/collaboration-groups/).
