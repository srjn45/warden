...
- **See what it costs** — `warden usage spend` prices each agent's real model usage into dollars
  with a budget gate (priced for the Claude backend; bring-your-own-model backends report
  tokens), and `warden usage savings` is an append-only ledger of the tokens warden keeps out of
...
- **Self-hosted and free** — one binary, a loopback REST API, and a JSON store on disk.
  ...
  `warden usage savings --calibrate`) send data only where you explicitly configure them.
- **Driven by an agent itself** — `warden daemon mcp` exposes the whole fleet as MCP tools, so an
...
HTTP clients to the daemon. `warden daemon mcp` is a stdio MCP server that bridges MCP tool calls to
...
- **Autopilot** — a goal-directed, long-running autonomous mode. Author a plan file (`autopilot.plan.yaml`), run `warden autopilot init` to scaffold the config, then `warden autopilot enable` to start. A **manager** agent (role `autopilot`) drives a fleet of **worker** agents (role `worker`, one per task) in isolated worktrees, gates their PRs through CI, and lands them into `autopilot/integration` — all without human intervention, spawning a **resolver** (role `brain`) on demand to unblock a stuck worker. A guardian heal loop keeps the manager alive through stalls, a daemon-internal overwatch nudges it to tend idle/waiting workers, and a cost-tier backend ladder escalates from free (Antigravity) to subscription (Claude, Codex) when rate-limited. The kill switch is `warden autopilot disable`. The switch is **per-repo** — `warden autopilot enable` enables only the current repository (add `--repo <root>` to target another), the enabled set is persisted so repos come back up across a daemon restart, and the manager marks its plan `status: complete` in place when it finishes so a done run is never re-run. See [Autopilot guide](https://srjn45.github.io/warden/guides/autopilot/) and [Autopilot concepts](https://srjn45.github.io/warden/concepts/autopilot/).
...
- **Token-savings ledger (`warden usage savings`)** — a real, append-only ledger of the tokens warden's lifecycle features keep out of agents' context, with an `--benchmark` A/B headline (without-vs-with warden, % reduction, $ saved) you can screenshot.
- **Cost governance (`warden usage spend`)** — the REAL model spend warden measures from agents' transcripts, priced per model into dollars and rolled up per-agent/repo/day (priced for the Claude backend; bring-your-own-model backends report tokens), plus a **budget gate** that softly warns before a spawn pushes daily/weekly spend over a configured `$` cap (a `$` column in `warden ls` and a live per-agent cost card on the web Metrics tab round it out).
...
- **Snapshots & insights** — `warden workspace snapshot` checkpoints a worktree + transcript for rollback; `warden usage insights` mines agent history for patterns and parallelization wins.
- **Branch tracking (`warden workspace branches`)** — opt-in monitor of each agent's CI status and standing vs `origin/main`, with non-blocking inbox/desktop alerts.
- **Extensibility** — a `warden project plugin` system (custom task types + lifecycle hooks over JSON-stdio) and an interactive **OpenAPI/Swagger UI** at `/api/docs`.
...
- **Remote access** — bearer-token auth (`warden daemon token …`) and a Docker/compose deployment.
...
> **Codex CLI** and **Antigravity CLI** are **β beta** — live-verified state, approval,
> and transcript fidelity, still maturing. The remaining non-`claude` backends
> (Aider, OpenCode, Crush, Goose, Cursor) are **experimental / work-in-progress** —
> functionality may be reduced or unverified.
...
first-class verbs — `warden git review` (Codex's diff reviewer), `warden backend model`
(Antigravity/Cursor's live model menu), and `warden agent fork` (branch a Codex
session into a new managed agent — see [`warden agent fork`](#warden-agent-fork)); see
[Agent-native superpowers](#agent-native-superpowers--warden-git-review--warden-backend-model).
See the design (`docs/superpowers/specs/2026-06-27-pluggable-agent-backends-design.md`, §5)
and roadmap item #52.

---

## Backend registry (`warden backend`)
...
```sh
warden agent adopt                          # newest session for cwd, resume under tmux
warden agent adopt --session-id <uuid>      # pick a specific Claude conversation
...
```sh
warden agent restore PROJ-350
```

### `warden agent recover`

The safety net for the tombstone reaper (which auto-archives a parent record once its whole sub-tree goes terminal): a stale `orphaned` status racing a daemon restart could previously let a genuinely-live session's record get archived out from under it. Bare `warden agent recover` scans **archived** records and only reports the ones whose tmux session is confirmed still alive; `--apply` re-inserts each candidate into the active store under its original id — any children (linked via `parent_id`, untouched by archiving) reconnect automatically.

```sh
warden agent recover                # report candidates only (dry run)
...
### `warden agent handoff --retire` (self-rotation; alias `warden agent rotate`)
...
### Shared context & messaging — `warden context` / `warden message`
...
warden context set build.status "green" --as agent-4f2a
warden context get build.status
warden context list --prefix build.

# Directed messages: per-agent inbox; sending wakes a parked (idle/waiting) agent
warden message send agent-9c1d "the API contract changed — re-check your client"
warden message inbox --as agent-9c1d
warden message wait --as agent-9c1d --timeout 120   # block in the daemon until a message arrives
```

### File-conflict detection — `warden workspace`
...
```sh
warden workspace conflicts                 # current cross-agent file conflicts
...
```sh
warden git review                  # the backend reviews its OWN uncommitted diff and streams findings
warden git review --base main      # review the branch's changes against a base instead
warden git review --prompt "focus on error handling"
warden git review --json           # neutral machine-readable findings: {summary, verdict, findings[]}
...
```sh
warden project memory                  # show the resolved path + the rendered view injected into agents
warden project memory --raw            # print the file verbatim
warden project memory --edit           # open it in $EDITOR (auto-creates it first)
warden project memory --path           # just the resolved path (scriptable; no auto-create)
```
...
```sh
warden inspect search auth backend          # AND every term across active sessions; --closed folds in the archive
warden inspect history --since 7d --type development --limit 20   # browse the archived (closed) store
```
...
### `warden inspect export` / `warden inspect import`

Serialize session **metadata** (not worktrees/branches/tmux) to JSON for backup or moving between machines.

```sh
warden inspect export > fleet.json           # active records; --all also dumps the archive
warden inspect import < fleet.json           # idempotent by id; --merge overwrites colliding records
```
...
```sh
warden inspect audit                                  # newest last (default tail 50)
warden inspect audit --action spawn --since 24h --json
warden inspect audit --target PROJ-350 --tail 0       # 0 = all
```

### `warden project preset save|list`

Save reusable spawn defaults under a name and replay them with `warden start --preset <name>`.

```sh
warden project preset save fast --model haiku --permission-mode acceptEdits --auto-restart
...
### `warden project prompt-template save|list`
...
### `warden project library list|save-preset|save-prompt`
...
```sh
warden project library list                               # presets + prompt templates + pipeline templates, labeled sections (alias: wd project library list)
warden project preset save fast --model haiku     # delegates to `warden project preset save`
warden project prompt-template save bugfix --prompt "Fix {{FILE}}"   # delegates to `warden project prompt-template save`
```
...
```sh
warden inspect resources                          # live snapshot
warden inspect resources --history                # per-agent performance history + anomaly warnings
warden inspect resources --history --agent PROJ-350
```
...
warden usage spend                     # same as `warden usage spend` (all flags wired through)
warden usage savings --benchmark       # same as `warden usage savings --benchmark`
```

Purely additive: it reuses the existing spend rollup and savings ledger and their render logic, so the top-level `warden usage spend` and `warden usage savings` keep working unchanged (they remain available as aliases). Resource footprint — memory/CPU/pressure — is a different axis; see `warden inspect resources`.

### `warden usage savings`

Read back the **token-savings ledger** — the tokens warden's lifecycle features kept out of agents' context windows. A real, append-only record, not an estimate. Gated by the `savings` config setting (default on). Also reachable as `warden usage savings`.

```sh
warden usage savings                        # per-feature table (saved/raw tokens, events)
warden usage savings --benchmark            # headline A/B: without-vs-with warden, % cut, $ saved, trend sparkline
warden usage savings --since 7d             # scope to a window (24h/7d/2w) or a date
warden usage savings --json                 # structured summary
...
```sh
warden workspace branches                       # table of per-agent CI + base-branch standing
warden workspace branches --json
```
...
```sh
warden approval auto set agent-abc123 on
warden approval auto set agent-abc123 off
```
...
today. The role set is a **fixed built-in catalog** (no user-defined roles). Each role
also carries a **default model tier** that feeds the [tiered router](#tiered-model-routing) when
nothing more specific pins the tier:
...
`auto-merger` are no longer first-class roles — that work is now a **task**
(`pr-review`/`development`/`merge-pr`, see [Tiered model routing](#tiered-model-routing)) —
but the three names still resolve to the `worker` role for back-compat.
...
- **`--task <name>`** — *what the agent is doing*, from the built-in **task
  registry**. Each task carries a tier: tier-1 (`analysis`, `architecture`,
  `design`, `research`, `spike`), tier-2 (`code-review`, `development`, `docs`,
  `pr-review`), tier-3 (`debug-ci`, `merge-pr`, `monitor-ci`, `release`). This is
  the canonical task→tier source.
...
> [worktrees & task types](#warden-workspace)); `--task` decides the model tier.
> The two name-sets overlap but are independent. **Surfaces:** `--tier` and
> `--task` are on `warden start` and the REST spawn body; `--tier` is also a
> pipeline-job field (`tier:`, alongside `role:`). Over MCP, `spawn_agent` routes
> by `role` only (its default tier feeds the resolver) — no `tier`/`task` params.
> See
> [`docs/specs/tiered-model-routing.plan.md`](docs/specs/tiered-model-routing.plan.md)
> and [`docs/specs/agent-roles.md`](docs/specs/agent-roles.md) for the full model.
...
```sh
warden workspace                       # list warden-owned worktrees (same as `worktree list`)
...
Only backends with a native session fork are forkable — **Codex** today (`codex fork`); forking one without (e.g. Claude) reports a clean "cannot fork". The source's backend session id must already be pinned (let it run a turn first). The fork inherits the source's repo + backend; `--type` defaults to `development`. Unlike `warden git review` / `warden backend model`, fork is a managed spawn that crosses the daemon, so it has **MCP + CLI parity** — the `fork_agent` MCP tool is the orchestrator twin (a thin wrapper over `spawn_agent` with `fork_from` set, no new endpoint).

### `warden daemon token generate|show|rotate`

Manage the bearer token that gates non-loopback (remote) access — see [Remote access](#remote-access).

```sh
warden daemon token generate                 # mint a 256-bit token (persisted to ~/.warden/token.env, 0600)
warden daemon token show                     # print the current token (paste into a remote client)
warden daemon token rotate                   # regenerate in place + restart the daemon
warden daemon token show --readonly          # print the read-only token (WARDEN_READONLY_TOKEN)
```
...
state-changing action and the interactive attach. Mint one with `warden daemon token
...
```sh
warden project plugin list                    # registered plugins: paths, custom task types, subscribed hook events, config errors
```
...
```sh
warden daemon mcp
warden daemon mcp --addr 127.0.0.1:8765
```
...
`warden daemon mcp` connects to the daemon at the `addr` config setting (default `127.0.0.1:8765`); to point it elsewhere use `"args": ["mcp", "--addr", "host:port"]`.
...
- **Metrics tab (`/metrics`)** — a scrollable column of uPlot chart cards: **CPU per agent**, **Memory per agent** (GiB), **Cost per agent** (live measured model spend in $, with a total/today/this-week headline and a sorted per-agent cost bar chart — top-N costliest, rest folded into an `others` row), **Context per agent** (a client-accumulated time series of each agent's live context fill, legend dot colored by `ok`/`warning`/`critical`; in-session only — resets on full reload), **Number of agents** (fleet size over time), and **Tokens saved** (daily bars from the savings ledger + a headline saved-tokens/$ figure). When the savings ledger is disabled the savings/cost cards show a "set `savings: true`" hint instead of an empty chart. A **Live footprint** card carries the former Resources panel.
