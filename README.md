...
- **Web** — real URL routing (`/cockpit`, `/tui`, `/pipelines`, `/metrics`, `/archive`, `/others`, `/agent/<id>` — deep-linkable, back/forward, shareable), a Cockpit home with a Fleet header, a **TUI tab** that streams the literal `warden tui` (the full three-pane cockpit — same panes, shortcuts, real shells and Claude Code) into the browser so you can drive the fleet from any device exactly as you do locally, a dedicated **Metrics** tab (per-agent **and** fleet-total CPU / memory, per-agent context, fleet size, tokens saved — two columns on desktop, single column on mobile), dark-mode theming, global keyboard shortcuts, Cockpit agent grouping, and an Archive tab plus an Others catch-all (last).
