...
| **File-conflict detection** (`warden collab`) | The daemon polls each active agent's worktree with `git diff` and warns (via the inbox, deduplicated) when two agents are editing the same file. Inspect with `collab conflicts` / `collab who-is-editing <file>`, `GET /collab/conflicts`, or the `get_collaboration_status` / `who_is_editing_file` MCP tools. Tunable via `collab_enabled` / `collab_interval`. |
