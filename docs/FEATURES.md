...
| Feature | Description |
|---|---|
| **URL-routed shell** | Tabs are real URLs via the History API — `/cockpit` (home), `/tui`, `/pipelines`, `/metrics`, `/archive`, `/others` (catch-all, last), and `/agent/<id>` per pinned agent. Back/forward, refresh, and shareable deep links all work; `/` redirects to `/cockpit`. |
| **TUI tab** | A full-window terminal that streams the **literal `warden tui`** — the daemon builds a shared three-pane cockpit (agent list + master shell/REPL + detail pane) and bridges it to the browser over the same WebSocket PTY as a per-agent attach (xterm.js). Every TUI keybinding (`enter/n/o/s/a/i/x/D/r/?/j/k/g/G`), the real shell (with its own autosuggestions/tab-completion), and the real Claude Code in the detail pane behave exactly as they do locally — from any device. Shared across clients (`window-size latest` lets the active client drive sizing); built lazily on first attach. Shift+Enter is mapped to Claude Code's newline; a few browser-reserved chords (`Ctrl+T/W/N`) are the only fidelity gap, mostly reclaimed by installing the PWA. |
