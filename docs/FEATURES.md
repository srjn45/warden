...
| **Reactive backend hard-limit recovery** | When an agent hits a confirmed provider hard limit (session, weekly, or monthly cap), the daemon automatically tries the next eligible subscription backend/model from the backend registry — no operator action needed. **Per-backend rate-limit detection** works for all eight agent backends: pane-based `RateLimitDetector` hooks for Claude, Codex, Cursor, Antigravity, and Aider; a ~60 s **usage-API polling fallback** (`limitSessionsFromSnapshot`) for Tea TUI backends (OpenCode, Crush, Goose) whose panes carry no parseable limit text. Usage windows are read from `internal/backendusage`; each candidate `(backend, model)` pair is ranked by minimum known headroom across overlapping pools (Antigravity/Gemini vs non-Gemini, Codex short + weekly windows, etc.). A candidate must **stabilize** — live non-limited status for at least `rate_limit.recovery.stabilization_window` (default 10 s) — before recovery clears; process launch alone is not success. On an immediate hard limit from the new candidate, the coordinator records the attempt and advances to the next candidate in the same generation. When all candidates are exhausted, the session persists `waiting_for_capacity` with known reset times and retries on the earliest reset or the fallback interval. A manual `warden switch`/stop/delete always supersedes automatic work; stale timers and late callbacks cannot resurrect it. Pipeline and Autopilot ownership, worktree, branch, tags, and parentage are fully preserved through recovery. Recovery state (`phase`, `current`, attempts, `next_retry_at`) is exposed on every agent status endpoint, MCP `get_agent`/`list_agents`, and SSE stream. Config: `rate_limit.recovery.enabled` (default `true`) and `rate_limit.recovery.stabilization_window`. The deprecated `threshold_percent` and `rolling_quota_threshold` handover-settings fields are decoded for one compatibility window but have no effect — reactive recovery replaced proactive quota prediction entirely. Recovery is **universal** (not gated on handover settings). See [Backend recovery guide](https://srjn45.github.io/warden/guides/backend-recovery/). |
...
| **Agent sub-trees** | Agents spawned by another agent nest under their parent as a collapsible sub-tree (`▸ / ▾`, indented per depth); a cross-project child surfaces under its own project with a `↳ from <parent>` backlink. Deleting a parent with live children keeps it as a muted **terminated tombstone** header (`terminated · N running`) with no attach pane, so children never orphan; the daemon reaps the tombstone once the whole sub-tree goes terminal. `Enter` on a finished agent or tombstone opens its stored detail instead of attaching to a dead session. |
| **Project tree** | The Projects-tab navigator is fed by the shared **`internal/tree`** service (same hierarchy as `GET /api/v1/tree` / SSE `tree`): projects → autopilot runs → pipelines → agents, with collapse/cursor keyed on composite node ids. Anything with no project lands under **No project**. Open projects always show (even empty); hibernated projects are hidden and their agents re-homed into No project. |
...
| `commit` / `push` / `sync` | Git lifecycle on the agent's worktree (or an explicit validated worktree in the same repository) — staged commit (auto-message when omitted), push (`force` → `--force-with-lease`), rebase-sync — returning compact structs instead of raw git output (see §22) |
...
| `rate_limit.recovery.enabled` | `true` | Toggle the reactive hard-limit recovery engine. When enabled, a confirmed `StatusRateLimited` transition starts the coordinator rather than only the legacy same-backend auto-resume. Disable to fall back to the legacy auto-resume-only behaviour |
| `rate_limit.recovery.stabilization_window` | `10s` | How long an agent must stay in a non-rate-limited live status (working/idle/waiting-for-input) before the candidate is declared stable and recovery cleared. Go duration string (e.g. `30s`, `2m`). Process launch alone is insufficient — the window must elapse |
