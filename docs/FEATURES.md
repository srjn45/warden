...
| **Agent roles** | A **role** is a named, persistent system-prompt **persona** attached to an agent, plus a set of default spawn flags. CLI: `--role <name>` on `warden start`; MCP: `role` on `spawn_agent`; UIs: TUI new-agent `ctrl+r` picker, web **+ New agent** Role dropdown. Fixed built-in catalog (no user-defined roles): `general` (default, no persona), `orchestrator`, `implementer`, `auto-merger`, `reviewer` — browse with `warden role list` / MCP `list_roles`. Each role carries a persona and default flags (`type`/`model`/`permission_mode`/`auto_approve`/`tags`) that fill only the fields the caller left unset (explicit request > role default > global default; tags are unioned; `auto_approve` OR-ed). Switch a running agent's role with `warden set-role <id> <name>` / MCP `set_role` — it persists the name and **relaunches** the agent so the new persona re-injects (its in-flight turn is discarded); `general`/empty clears the persona. Only the role **name** is stored in the session (`Session.Role`, empty ⇒ `general`, back-compat — no store migration); the persona re-resolves from the `internal/role` registry at every (re)launch, so nothing persona-shaped is persisted. Injected through the same seam as warden's collab/git/pipeline hints — Claude via `--append-system-prompt` (file-backed, never inlined on the launch line), the injecting backends (Codex/OpenCode/Cursor/Antigravity/Crush/Goose) prepended into their rules file; Aider (no injection seam) degrades silently. An empty persona injects nothing, so a plain/`general` spawn is byte-identical to before roles existed. |
...
| Feature | Description |
|---|---|
| **Pipelines** (`warden pipeline`) | YAML-defined **DAG of dependent agent jobs**. The daemon runs them: dependency-free jobs start first, each job's `emit` publishes output and unblocks dependents — keeping the lead orchestrator agent off the critical path. Sub-commands: `validate` (client-side spec check, CI-friendly exit codes), `create` (from a spec file **or** a built-in `--template`), `list-templates`, `start`, `pause`/`resume` (halt new spawns while in-flight jobs finish), `show`, `list`, `edit-job`, `retry`, `cancel`, `delete`. Drivable from the CLI, the MCP tools (create/start/show/cancel/list), and with full TUI + web visibility (DAG view). A pipeline is warden's shared **lifecycle substrate** at three autonomy levels — **direct** (a human runs `create_pipeline`; no orchestrator, no tokens), **delegated** (see below), and **autopilot**. |
| **Delegated monitoring** (push, not poll) | The token unlock for orchestrators. When an orchestrator calls `create_pipeline`, warden records it as the pipeline's **`owner_id`** (the pipeline-mode analogue of a spawned agent's `parent_id`). Set spec-level **`notify_owner: true`** to subscribe the owner, and mark decision-point jobs **`callback: true`**; warden then **push-wakes** the owner via a directed message (read with `wait_for_message`) at each callback job **and once** when the pipeline reaches a terminal state (done/stalled). The orchestrator **blocks instead of polling** and spends tokens only at real decision points. A **pure DAG** marks no callbacks — one wake, at the end. |
| **Worker done-signal** (`warden job done`) | A pipeline worker declares its own completion in **one shot** — `wd job done --summary "…" [--status success\|failure\|blocked]` — so warden captures **status *and* summary** without spending a follow-up interrogation turn. Inside a job `WARDEN_PIPELINE_ID`/`WARDEN_JOB_ID` are injected automatically; run outside a pipeline it degrades to printing a `<<WARDEN_DONE>>{json}` **sentinel line** the lifecycle watches (the backstop for backends that can't run a command mid-task). Complemented by out-of-band **stuck detection** — a pane-state classifier (`working`/`idle-at-prompt`/`blocked-on-approval`/`exited`, zero-token) plus a wall-clock watchdog that flags a hung worker; a worker that has *finished and sits at an idle prompt* classifies as idle/done, **not** `needs-input`. |
| **Collaboration groups** (`warden collaborate group`) | A named set of **per-project orchestrator agents** made mutually discoverable so they message/delegate across projects — a **roster + introduction broker**, not a new channel (peers talk over the existing directed-message bus). `warden collaborate group <name> join\|leave` (MCP `collaborate_group {group, action}`). **join** creates the group if absent, enforces **one orchestrator per project** (project = canonical git-remote key, with a `local:` fallback for remoteless repos; a duplicate join returns **409** naming the seated agent), switches the caller to the **orchestrator** role, resolves a one-line project **summary** (declared blurb in `CLAUDE.md`/`README.md` → else asked once → cached), and **brokers introductions both directions** at zero agent tokens. **leave** is **soft** — the agent stops being discoverable and accepts no new inbound work, but in-flight replies still route by agent-id; warden notifies peers. **Terminate** (not leave) is the hard, work-orphaning path and requires explicit confirmation naming the group(s). On `recover_agents` a recovered orchestrator **auto-rejoins** its groups (group durable; membership live). Roster record is lean — `{name, id, project-key, summary, joined_at}`, never transcripts. |
...
| Feature | Description |
|---|---|
| **Control pane — projects-first** | Polls the daemon ~1×/sec; browse with `↑`/`↓` without disturbing the viewports. Two bordered inner frames: **Projects** and **Terminals**. The old top-level **Approvals and Pipelines sections are removed** — prompts surface via an agent's needs-input status (`p` answers), pipelines render inside Projects. Each agent row carries a compact **backend** token (claude/aider/…); the full **agent info** pane (`i`) lists every stored field — backend, model, role, tags, context, location, refs, rate-limit, lifecycle, plumbing, last pane excerpt. An agent with no recorded backend renders as **claude**. |
| **Projects frame** | Agents are grouped under their **Project** node — the agent's worktree's git project, keyed by the canonical remote URL (with a `local:` fallback for remoteless repos; **the same normalizer as collaboration groups**). Multiple worktrees of one repo collapse to a single project node. Top-level agents list directly under the project; each agent's subagents and pipelines nest beneath it. The "dir" vocabulary is renamed **Project** throughout (titles, headers, hints, help). Cursor-home lands on the first agent under the first project. |
| **Pipelines inside Projects** | There is no top-level Pipelines home. A **delegated** pipeline (carrying an owner link) nests **under its owning orchestrator**; a **human/orchestrator-less** pipeline renders **under its project node** (sibling to that project's agents). Job rows keep collapse-completed-by-default and the cancel affordance; `r` retries a failed job. |
| **Approvals via needs-input** | No top-level Approvals section — a worker blocked on a prompt surfaces through its agent node's **needs-input** status; `p` (or `Enter` on the ⏳ row) opens the answer overlay. A worker that has *finished and sits at an idle prompt* correctly shows **idle/done**, not `needs-input`. |
| **Agent sub-trees** | Agents spawned by another agent (`spawn_agent`) nest under their parent as a collapsible sub-tree (`▸ / ▾`, indented per depth, `h`/`l` to toggle) inside the parent's project. Deleting a parent with live children keeps it as a muted **terminated tombstone** header (`terminated · N running`) with no attach pane, so children never orphan; the daemon reaps the tombstone once the whole sub-tree goes terminal. `Enter` on a finished agent or tombstone opens its stored detail instead of attaching to a dead session. |
| **Terminals frame + terminal pane** | Terminals stay a **flat, non-project-scoped** frame (a `cd` inside a terminal roams across projects, so a terminal has no fixed project). First-class `kind=terminal` sessions shown in the bottom-left **terminal** pane. A **default terminal** opens in the launch directory at startup, and the cockpit always keeps at least one. Names update live as the shell `cd`s (`<index>. <repo>:<rel>/ (<branch>)`). Press **`t`** to create/focus a terminal in the opened agent's directory (`(c)reate`/`(f)ocus`); `x` closes one. |
| **Open Project panel (`o`)** | `o` takes over the whole control pane with a full-pane **Open Project** panel: (1) a persisted **recent-projects** list; (2) **open local** (`l`) via a directory navigator; (3) **open via git** (`g`) — clone into `~/.warden/workspace/<project>` (name collisions disambiguated). Opening any project **auto-spawns its single orchestrator**, enforcing **one-orchestrator-per-project** (an existing orchestrator is *focused*, not duplicated — the same invariant `collaborate group join` uses). `Esc` returns to the Projects view. |
...
| **In-cockpit actions** | `n` new agent, `t` new/focus terminal, `o` Open Project panel, `s` send, `a` attach (full-screen), `d` digest overlay, `i` agent/pipeline detail, `e` event log, `p` approvals, `b` backends, `ctrl+a` autopilot, `c` context/message inspector, `r` retry job, `x` kill/cancel/close, `D` delete pipeline record, `?` help. |
| **Viewport rotation** | Global **Alt** bindings (work from any pane, even while typing): `Alt+t` cycles the terminal pane over terminals, `Alt+a` cycles the agent pane over the **Projects frame's** agents. Add **Shift** (`Alt+Shift+t/a`) to rotate in reverse. `Alt+p` (pipeline-agents) is **retired** — pipelines are reached inside the Projects tree. Each rotation grabs focus on the pane it drives. A config-free **`Ctrl-b` prefix fallback** (`Ctrl-b` then `t`/`a`, Shift for reverse) runs the same rotation for terminals that don't send Alt/Option as Meta — **macOS Terminal.app / iTerm2** by default. |
...
| Feature | Description |
|---|---|
| **URL-routed shell** | Tabs are real URLs via the History API — `/cockpit` (home), `/agents`, `/pipelines`, `/terminals`, `/metrics`, `/archive`, `/others` (catch-all, last), and `/agent/<id>` per pinned agent. `/tui` is a full-screen route (launched from the top bar, not a tab). Back/forward, refresh, and shareable deep links all work; `/` redirects to `/cockpit`. |
| **Agents tab** | An `/agents` fixed tab mirroring the Terminals tab's master-detail layout: the live agents list on the left (the `kind`-filtered agents list), the focused agent's interactive view on the right — selecting an agent opens it, exactly like Terminals. Web stays **flat/tab-based** this wave — the projects-first grouping and Open Project panel are TUI-only; the standalone **Pipelines** tab is untouched. |
...
the fleet through tool calls. **93 tools** are exposed — every fleet/data feature
...
| `collaborate_group` | Join/leave a collaboration group (`{group, action}`) — one orchestrator per project, warden-brokered intros, leave-soft |
