# Antigravity CLI backend (stable)
...
| `UsageLimits`          | ✅    | Provider quota tracked via `retrieveUserQuotaSummary` (same as `agy /usage`): four windows — Gemini/non-Gemini × 5-hour/weekly — with live usage % and reset times. |
