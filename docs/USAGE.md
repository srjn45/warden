...
| **Antigravity CLI** | `antigravity` | A | 🧪 Experimental. Google-hosted free tier (`agy`, multi-vendor model menu); structured trajectory JSONL ⇒ real digests. **Resumes** dir-scoped (`agy -c`). Live state + `Do you want to proceed?` approval detection; context injection via `AGENTS.md` (`InjectContext`). No priced spend. See [`docs/agent-backends/antigravity.md`](../agent-backends/antigravity.md). |
