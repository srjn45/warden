...
| Spawn fails with `daemon error (503): request timed out` | In a very large monorepo `git worktree add` (a full working-tree checkout) can take minutes. Spawn (and commit/push/sync/check/prune/…) now get a 10-minute daemon budget instead of 30s. If a spawn is still cut, a partial worktree is now cleaned up automatically. Both budgets are configurable — `http_timeout_slow` (default `10m`) for lifecycle routes, `http_timeout_fast` (default `30s`) for everything else; raise the slow one if your repo's checkouts or hooks run longer. Ensure your daemon is up to date (rebuild + restart). |
