---
title: Running a fleet of Claude Code agents without losing your mind
description: I kept spawning more Claude Code agents than I could track. So I built warden — a single Go binary that spawns, watches, and prices a whole fleet from one cockpit.
head:
  - tag: meta
...
I'd gotten good at running Claude Code. So good that one agent was no longer
enough. I'd kick off a refactor in one terminal, a test-writing pass in another,
...
So I built **warden** — a single Go binary that runs a fleet of Claude Code
agents and gives you exactly one place to see all of them.

![warden in action: spawning and monitoring a fleet of Claude Code agents from one cockpit](/warden/media/hero.gif)
...
item. `warden spend` prices each agent's *real* Claude usage into dollars and
...
## It also drives Claude itself

This is the part that surprised people I showed it to. `warden mcp` exposes the
whole fleet as MCP tools. So an *orchestrator* Claude session can spawn agents,
query their status, and talk to a specific running agent — Claude managing a
team of Claudes, with warden as the substrate. You're not limited to driving it
by hand.
...
than one Claude Code agent at a time — or you're about to — I'd love for you to
