...
<style>{`
  .wbmdiag {
    ...
    --d-muted: #59616f; --d-border: #dee3ea;
    --d-good: #2f8f63; --d-good-soft: #e3f2ea; --d-good-ink: #1f6f4b;
    --d-warn: #b8791f; --d-warn-soft: #f6ecd8; --d-warn-ink: #8f5d13;
    --d-shadow: 0 1px 3px rgba(20,30,45,.06), 0 6px 20px -10px rgba(20,30,45,.14);
  }
  @media (prefers-color-scheme: dark) {
    :root:not([data-theme="light"]) .wbmdiag {
      --d-surface: #171b23; --d-surface2: #1e232d; --d-ink: #e7ebf1;
      --d-muted: #9aa4b4; --d-border: #2a313d;
      --d-good: #4fb283; --d-good-soft: #17281f; --d-good-ink: #7fd0a6;
      --d-warn: #d69b46; --d-warn-soft: #2a2113; --d-warn-ink: #e0ad63;
      --d-shadow: 0 1px 2px rgba(0,0,0,.3), 0 8px 24px -12px rgba(0,0,0,.55);
    }
  }
  :root[data-theme="dark"] .wbmdiag {
    --d-surface: #171b23; --d-surface2: #1e232d; --d-ink: #e7ebf1;
    --d-muted: #9aa4b4; --d-border: #2a313d;
    --d-good: #4fb283; --d-good-soft: #17281f; --d-good-ink: #7fd0a6;
    --d-warn: #d69b46; --d-warn-soft: #2a2113; --d-warn-ink: #e0ad63;
  ...
  .wbmdiag { margin: 1.5rem 0; }
  .wbm-tier { display: flex; align-items: center; gap: 10px; margin: 22px 0 10px; font-family: ui-monospace, monospace; font-size: 12px; letter-spacing: .07em; text-transform: uppercase; color: var(--d-muted); }
  .wbm-tier:first-child { margin-top: 0; }
  .wbm-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(220px, 1fr)); gap: 12px; }
  .wbm-card { background: var(--d-surface); border: 1px solid var(--d-border); border-radius: 10px; padding: 14px 14px 13px; box-shadow: var(--d-shadow); display: flex; flex-direction: column; gap: 7px; }
  .wbm-card.stable { border-left: 3px solid var(--d-good); }
  .wbm-card.exp    { border-left: 3px solid var(--d-warn); }
  .wbm-name { font-family: ui-monospace, "Cascadia Code", monospace; font-size: 16px; font-weight: 600; color: var(--d-ink); }
  .wbm-desc { font-size: 13px; color: var(--d-muted); line-height: 1.5; }
  .wbm-pill { align-self: flex-start; font-family: ui-monospace, monospace; font-size: 11px; font-weight: 500; letter-spacing: .04em; padding: 3px 8px; border-radius: 999px; text-transform: uppercase; }
  .wbm-pill.stable { background: var(--d-good-soft); color: var(--d-good-ink); }
  .wbm-pill.exp    { background: var(--d-warn-soft); color: var(--d-warn-ink); }
`}</style>

<div class="wbmdiag">
  <div class="wbm-tier"><span class="wbm-pill stable">Stable</span><span>4 backends</span></div>
  <div class="wbm-grid">
    <div class="wbm-card stable"><span class="wbm-name">claude</span><span class="wbm-pill stable">Stable</span><span class="wbm-desc">Full-capability reference backend — structured transcript, resume, approvals, priced spend, system-prompt injection.</span></div>
    <div class="wbm-card stable"><span class="wbm-name">codex</span><span class="wbm-pill stable">Stable</span><span class="wbm-desc">OpenAI Codex CLI. JSONL rollout transcript, session fork (<code>wd agent fork</code>), review command, dir-scoped resume with discover-then-pin.</span></div>
    <div class="wbm-card stable"><span class="wbm-name">cursor</span><span class="wbm-pill stable">Stable</span><span class="wbm-desc">Cursor CLI agent — live state, approval &amp; trust detection, triple-bucket subscription usage. No digest (Tier-C transcript).</span></div>
    <div class="wbm-card stable"><span class="wbm-name">antigravity</span><span class="wbm-pill stable">Stable</span><span class="wbm-desc">Google Antigravity (<code>agy</code>) — trajectory JSONL transcript, dual-bucket usage, multi-vendor model menu via OAuth.</span></div>
  </div>
  <div class="wbm-tier"><span class="wbm-pill exp">Experimental</span><span>4 backends</span></div>
  <div class="wbm-grid">
    <div class="wbm-card exp"><span class="wbm-name">aider</span><span class="wbm-pill exp">Experimental</span><span class="wbm-desc">Bring-your-own-model; autonomous one-shot task that exits when done. Structured Tier-A transcript; no resume.</span></div>
    <div class="wbm-card exp"><span class="wbm-name">opencode</span><span class="wbm-pill exp">Experimental</span><span class="wbm-desc">Structured Tier-A transcript (via <code>opencode export</code>) and dir-scoped resume; bring-your-own-model including local Ollama.</span></div>
    <div class="wbm-card exp"><span class="wbm-name">crush</span><span class="wbm-pill exp">Experimental</span><span class="wbm-desc">SQLite-backed Bubble Tea TUI; transcript via <code>crush session show --json</code>; dir-scoped resume; prompt auto-typed post-launch.</span></div>
    <div class="wbm-card exp"><span class="wbm-name">goose</span><span class="wbm-pill exp">Experimental</span><span class="wbm-desc">Interactive REPL agent; name-deterministic resume; transcript via <code>goose session export</code>; BYO provider via env vars.</span></div>
  </div>
</div>
