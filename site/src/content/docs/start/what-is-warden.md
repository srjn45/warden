...
| **Interactive REPL** *(experimental)* | `warden repl` — a [local-LLM conductor REPL](/warden/multi-agent/repl/) that turns plain-English intent into confirmed warden actions, spending no Claude tokens. | When you want NL control without an MCP Claude session. |
