...
Interactive mode **starts without a local model** — the `/` commands and `!`-shell always work. The natural-language half needs `local_llm: true` (and `local_llm_url`/`_model`/`_timeout`); without it, a bare line tells you so and points you at `/help`. Because execution is always plain warden API calls, the REPL spends **no Claude tokens**.
