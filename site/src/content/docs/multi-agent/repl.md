...
| **Local project grounding** | Ask a project question — `/memory <q>` (`/mem`/`/ask`) or the `project_memory` tool — and warden answers it **locally** from `.warden/memory.md` (`memory_ground`, default on). Read-only and `$0`: served on the **local model only** (no cloud round-trip), it cites each entry's trust (`unverified`/`trusted`/`human`) + provenance, degrades to the matching entries verbatim with no local model, and answers "not in project memory" for an absent/empty file (never auto-creating it). |
