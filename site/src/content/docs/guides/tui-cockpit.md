...
| **Project groups** | On the **Projects** tab, agents, pipelines, and autopilot runs nest under their project — a collapsible group header (`▸ / ▾`) reading the project **name** and its `~`-abbreviated path plus an agent-count badge. The hierarchy comes from the shared project-tree service (same as [`GET /api/v1/tree`](/warden/reference/project-tree/)). Open projects always show (even empty, IDE-style); work in a directory that is not yet a registered project clusters under a loose dir group; anything with no location falls into a **No project** bucket. `←` / `→` collapse / expand the group under the cursor; `x` **closes** a project (see below); `Enter` is reserved for future project details. `o` opens a project (Local / Remote / New), which also becomes the spawn target for `n`. |
